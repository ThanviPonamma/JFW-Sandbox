...
// to ge the cart item list from the apex method 'getCartItemsList' of class 'CopperShoppingCartItem_Apex'
import getCartItemsList from '@salesforce/apex/CopperShoppingCartItem_Apex.getCartItemsList_Apex'
// to ge the updated quantity for the selected pos items from the apex method 'updateQuantityForSelectedItem' of class 'CopperShoppingCartItem_Apex'
import updateQuantityForSelectedPosItem from '@salesforce/apex/CopperShoppingCartItem_Apex.updateQuantityForSelectedItem'
// to send the pos items to the apex method 'deleteCartItemFromShippingCart' of the class 'CopperShoppingCartItem_Apex' if user wants to delete the items
import deleteCartItemFromShippingCart from '@salesforce/apex/CopperShoppingCartItem_Apex.deleteCartItemFromShippingCart'
...
import CopperCaneNoImage from '@salesforce/resourceUrl/noimageavailable';
// to send add same address to all to apex method
import getSelectedUserAddressBookListForAllItems from '@salesforce/apex/CopperShoppingCartItem_Apex.getSelectedUserAddressBookListForAllItems'
// To check if the address is domastic or international
import getUserDetails from '@salesforce/apex/CopperShoppingCartItem_Apex.getUserDetails'

export default class CopperShoppingCartItemDetailsComponent extends LightningElement {
    ...
    CopperCaneNoImage = CopperCaneNoImage;
    @track invokeType='server'
    //to hold the list of pos items in cart along with the details
    @wire(getCartItemsList, { selectedBuyBookId: '$buyBookId' }) cartItemsToBeDisplayed;
    ...
    get cartItemDetails() {
        ...
        if (this.cartItemsToBeDisplayed.data) {
            ...
            for (var i = 0; i < posItemsListWithCart.length; i++) {
                ...
                // var availableBudget = this.selectedBuyBookDetail.Available_Budget__c - this.selectedBuyBookDetail.Consumed_Budget__c;
            ...
            if (this.totalQuantity > 0) {
                console.log('true--->',this.totalQuantity);
    ...
    //these details are sent to child component '-copper-shopping-cart-address-modal-component'
    ...
    calculatePrice() {
        ...
        for (var i = 0; i < cartItems.length; i++) {
            ...
            var availableBudget = this.selectedBuyBookDetail.Available_Budget__c - this.selectedBuyBookDetail.Consumed_Budget__c;
             /**The below 2 line are added for temporary as exceed budget needs to be removed*/
            this.disablePreviewButton();
            this.openPreviewPage();
