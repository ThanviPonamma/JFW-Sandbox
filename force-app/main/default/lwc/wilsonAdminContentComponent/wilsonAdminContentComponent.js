...
export default class WilsonAdminContentComponent extends NavigationMixin(LightningElement) {
     // to track the URL
  @track urlPath;
  ...
  handleEvent(messageFromEvt) {
    ...
    //checks if the menu selected is orderstatus
    if (this.selectedMenu == 'addressBook') {
    ...
    if (this.selectedMenu == 'managebrands') {
      ...
      this.isManageASN = false;
      this.isManageApproval = false;
      window.history.pushState("", 'Manage Brands', this.accountURL + '/s/' + 'managebrands');
    }
    // To navigate to Manage Item Type  page
    //checks if the menu selected is Manage Item type
    if (this.selectedMenu == 'manageitemtypes') {
    ...
    if (this.selectedMenu == 'manageadvanceshipmentnotice') {
