...
import addOrEditShippingMethod from '@salesforce/apex/JFWManageShippingMethods_Apex.addOrEditShippingMethod'

//to display Toast Message
import { ShowToastEvent } from 'lightning/platformShowToastEvent';


export default class JfwManageShippingMethodCreateEditComponent extends LightningElement {
...
handlChangeShippingMethodName(event){
 ...
 //to hold the value in the id field
  let shippingMethodName = shippingMethodInputFieldValue.value;
    console.log('in handle chnage shipping Method Input Field Value',JSON.stringify(shippingMethodName))
  ...
  //if the entered shipping methods name starts with space, the trim the same and store the value.
     ...
     var regexForSpecialCharacters = /[^a-zA-Z0-9-]/ig;
        ...
        if (regexForSpecialCharacters.test(shippingMethodName)) {
            console.log('enetered',shippingMethodName)
    ...
    handleChangeActive(event){
        ...
        // console.log('entred 95', this.shippingMethod.Active__c);
        if( this.shippingMethod.Active__c){
            this.isDefaultDisabled = true;
            // console.log('entred if',  this.isDefaultDisabled)

        }
        else{

            this.isDefaultDisabled = false;
            // console.log('entred else',  this.isDefaultDisabled)
        }
    }
 ...
 // On click event when the user does the changes saves the shipping method 
    saveShippingMethod(){
        // console.log('shipping method--->',JSON.stringify(this.shippingMethod));
