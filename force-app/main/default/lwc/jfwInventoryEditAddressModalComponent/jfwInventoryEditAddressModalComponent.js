...
// To get the List of updated address from the apex class "JFWInventoryAddressBook" along with the method "updateAddressBook"
import updateAddressbook from '@salesforce/apex/JFWInventoryAddressBook.updateAddressBook'

export default class JfwInventoryEditAddressModalComponent extends LightningElement {
...
/**Authors: Thanvi & Nachappa
   * Date : 06-01-2020
   * CRNo : JFW-06-Jan-05 
   * The below line is commented to meeth requiremts of the CR
   * The ship to company and city combination has to be auto filled for Destination Name
  //To validate the DESTINATION NAME 
  handleChangeDestinatioName(event) {
    // to hold the destination name with the help of class name
    let destinationInputField = this.template.querySelector(".addressBookDestinationName");
    // to hold the destination name value
    let destinationNameValue = destinationInputField.value;
    // to hold the regEx
    var regexForSpecialCharacters = /[^a-zA-Z0-9]/;
    if (destinationNameValue.trim() == '') {
      destinationNameValue = destinationNameValue.trim();
    }
    if (regexForSpecialCharacters.test(destinationNameValue)) {
      var destinationNameValue_FirstCharacter = destinationNameValue.match(/[a-zA-Z0-9]/);
      if (!destinationNameValue_FirstCharacter) {
        destinationNameValue = '';
      } else {
        var destinationNameValue_Index = destinationNameValue.indexOf(destinationNameValue_FirstCharacter);
        destinationNameValue = destinationNameValue.substring(destinationNameValue_Index, destinationNameValue.length);
      }
    }
    //to update the destination value
    this.updatedAddressBook.Destination_Name__c = destinationNameValue;
    // to reort error if there is.
    destinationInputField.reportValidity();
    // to call the validate addres book method.
    this.validateAddressBook();
  } */
/**Authors: Thanvi & Nachappa
   * Date : 06-01-2020
   * CRNo : JFW-06-Jan-05 
   * The below line is added to meeth requiremts of the CR
   * The ship to company and city combination has to be auto filled for Destination Name**/
  //To capture the DESTINATION NAME 
  handleChangeDestinatioName(event) {
    if(this.updatedAddressBook.Shipto_Company__c!= undefined && this.updatedAddressBook.City__c!=undefined)
    this.updatedAddressBook.Destination_Name__c = this.updatedAddressBook.Shipto_Company__c+'-'+this.updatedAddressBook.City__c;
  }
  validateEmailOnkeydown(event) {
    if (event.keyCode == 32)
      event.preventDefault();
  }


  // Onchange event when the users enters the ship to name in the input field this event is invoked
  ...
  // Onchange event when the users enters the ship to company in the input field,event is invoked
  ...
  * The ship to company and city combination has to be auto filled for Destination Name**/
   ...
   * Date : 06-01-2020
   * CRNo : JFW-06-Jan-05
   * To autofill destination name with the combination of ship to company and city */
...
/**Authors: Thanvi & Nachappa
   * Date : 06-01-2020
   * CRNo : JFW-06-Jan-05 
   * The below is commented to meeth requiremts of the CR
   * The ship to company and city combination has to be auto filled for Destination Name
  // Onchange event when the users adds their city in the input field,event is invoked
  handleChangeCity() {
...
//to remove the special characters for zip code
  ...
  SaveAddressBook(event) {
    //console.log('selectedAddressBook--->',JSON.stringify(this.updatedAddressBook));
    updateAddressbook({
