...
// importing the method addOrEditPOSItem from the class JFWManagePosItems_Apex
import addOrEditPOSItem from '@salesforce/apex/JFWManagePosItems_Apex.addOrEditPOSItem'
// importing the method getStateList from the class JFWManagePosItems_Apex
import getStateList from '@salesforce/apex/JFWManagePosItems_Apex.getAllState'
// importing the method getAllAndSelectedStates from the class JFWManagePosItems_Apex
import getAllAndSelectedStates from '@salesforce/apex/JFWManagePosItems_Apex.getAllAndSelectedStates'
...
export default class JfwManagePosItemCreateEditComponent extends LightningElement {
    ...
    @track today;
    // Authors: SN,TP Date: 31-12-2020 CR No: JFW-28-Dec-03 
    ...
    /** Authors: Sanjana & Thanvi Date: 31-12-2020 CR No: JFW-28-Dec-03 
     * The below code is commented to meet requirements of the CR.
    ...
    connectedCallback() {
        ...
        // console.log('posItems in callBack------>', JSON.stringify(this.posItems));
    ...
    /** Authors: Sanjana & Thanvi Date: 31-12-2020 CR No: JFW-28-Dec-03 
     * The below code is added to meet requirements of the CR.
     * To capture the positem id and to determine if the item selected to edit belongs to item type name - coupon. */
    // init method to hold the result from parent component and display on page load
    connectedCallback() {
        // holding the list of pos item details of the partcular pos item id fetched fom the parent
    ...
    /** Authors: Sanjana & Thanvi Date: 31-12-2020 CR No: JFW-28-Dec-03 
     * The below code is commented to meet requirements of the CR. 
     * The section to select the states needs to be displayed to the user when the item type is coupon.
    ...
    handleChangeForItemType(event) {
        ...
        // console.log('itemTypeName',itemTypeName.toUpperCase());
        //to check if the item name is COUPON
        if (itemTypeName.toUpperCase() == 'COUPON') {
            this.posItems.posItemList.Length__c = "0.003";
            this.posItems.posItemList.Width__c = "0.001";
            this.posItems.posItemList.Height__c = "0.006";
            this.posItems.posItemList.Weight__c = "0.005";
            //capture the positemId
            this.posItemId = this.posItems.posItemList.Id
            //Enable the select state section (DUAL LIST)
            this.displayStateListDualBox = true;
            // console.log('calling',this.posItemId);
        ...
        //  Authors: Thanvi and Vanditha
        // Date:08/01/2021
        // CR No : JFW-08-Jan-06 
        // The below lines are added to met the requirement of the CR  
        // Autofill the dimensions on selection of the item type disable the dimensional(length,width,height,weight) 
        if (itemTypeName.toUpperCase() == 'RACK') {
            this.posItems.posItemList.Length__c = "2";
            this.posItems.posItemList.Width__c = "3";
            this.posItems.posItemList.Height__c = "4";
            this.posItems.posItemList.Weight__c = "5";
        }

        if (itemTypeName.toUpperCase() == 'POS') {
            this.posItems.posItemList.Length__c = "0.01";
            this.posItems.posItemList.Width__c = "0.02";
            this.posItems.posItemList.Height__c = "0.03";
            this.posItems.posItemList.Weight__c = "0.09";
        }

        if (itemTypeName.toUpperCase().localeCompare('​​​​​​​BRANDED MERCHANDISE') == 0) {
            this.posItems.posItemList.Length__c = "0.7";
            this.posItems.posItemList.Width__c = "0.8";
            this.posItems.posItemList.Height__c = "0.6";
            this.posItems.posItemList.Weight__c = "0.9";
        }
        if (itemTypeName.toUpperCase() == 'ALL ITEM TYPES') {
            this.posItems.posItemList.Length__c = "0.001";
            this.posItems.posItemList.Width__c = "0.001";
            this.posItems.posItemList.Height__c = "0.001";
            this.posItems.posItemList.Weight__c = "0.001";
        }
    ...
    validatePosItemName() {
        ...
        if (posItemName.length > 40) {
            posItemNameInputField.setCustomValidity("Please enter item name less or equal to 40 characters.");
        }
        if (posItemName.length == 4) {
    ...
    validatePosItemPrice(event) {
        ...
        // console.log('event.key parent--->',JSON.stringify(event));
        //invoke the method validatePosItemData
        this.validatePosItemData();
    }

    //Authors: Sanjana, Thanvi
    //Date: 12/03/2021
    //Aim: Commenting the below and rewriting it to satisfy the CR - JFW-12-Mar-12
    // validateItemDescription(event) {
    //     //assign the value of event.target.value to the field Description__c of the pos item
    //     this.posItems.posItemList.Description__c = event.target.value;
    //     //invoke the method validatePosItemData
    //     this.validatePosItemData();
    // }
    ...
    // Author: Sanjana, Thanvi
    //Date: 23/3/2021
    //Commenting the below lines and rewriting the method to satisfy the CR - JFW-22-Mar-13  
    // POS Item Image
    ...
    //     var posItemImageId = this.template.querySelector(".posItemFileId");
    //     //holds the array of valid file types
    //     var fileExtensionsToProceed = ['JPG', 'JPEG', 'PNG'];
    //     //if the file is not undefined
    ...
    //         //fetch the name of the extension of the file
    //         var imageFileExtension = imageName.split('.').pop();
    //         //if there is no extension for the file, assign null to the variable imageFileExtension
    //         if (!imageFileExtension)
    //             imageFileExtension = '';
    //         //allow the uploaded file of its of the type JPG, JPEG or PNG
    //         if (fileExtensionsToProceed.includes(imageFileExtension.toUpperCase())) {
    //             // check if the size of the image is more than 50KB, if so, display the custom message asking 
    //             //the user to upload a file that is less that 50 KB
    //             if (parseInt(imageSize) > 50480) {

    //                 posItemImageId.setCustomValidity("Image size should be less than 50 KB");
    //                 //assign true value to the variable isImageValidated
    //                 this.isImageValidated = true;
    //             } else {
    //                 console.log('entered else---->');
    //                 // invoke the method uploadHelper
    //                 this.uploadHelper();
    //             }
    //         } else {

    //             posItemImageId.setCustomValidity('.' + imageFileExtension + ' can not be accepted,Only accepts .png,.jpg or .jpeg');
    //             //assign true value to the variable isImageValidated
    //             this.isImageValidated = true;
    //         }
    //         //display error message if the uploaded file is invalid
    //         posItemImageId.reportValidity();
    ...
    //             console.log('image src---->', JSON.stringify(img.src));
    ...
    ResizeImageToUpload() {
            ...
            this.image.onload = (() => {
                ...
                if (parseInt(imageWidth) > 200 || parseInt(imageHeight) > 200) {
                  //creating a canvas
                   ...
                   // access elements rendered by a component using the class
                    ...
                    // const scaleSize = MAX_WIDTH / imageWidth;
                ...
                //else, preveiw the uploaded image
                   ...
                   //access elements rendered by a component using the class
                    ...
                    this.isImageValidated = false;
                    //check if the user is editing the pos item
                   if (this.isEditClicked == true) {                                                                                                                                             
                                        //if user is editing, enable the save button
                                        this.isButtonDisable = false;
                                    } else {
        ...
        savePosItem() {
            //to invoke the apex method to save the pos item
            // console.log('posItem as parameter---->', JSON.stringify(this.posItems.posItemList));
            // console.log('selectedStateIds as parameter---->', JSON.stringify(this.selectedStateIds));
            /** Authors: Sanjana & Thanvi Date: 01-01-2020 CR No: JFW-28-Dec-03
       * The below code is commented to send the state ids to be stored in the junction object for the item type name - Coupon
        addOrEditPOSItem({
            ...
            /** Authors: Sanjana & Thanvi Date: 01-01-2020 CR No: JFW-28-Dec-03
             * The below code is added to send the state ids to be stored in the junction object for the item type name - Coupon
             */
            addOrEditPOSItem({
                    posItem: this.posItems.posItemList,
                    imageBlobData: this.isImageValidated ? '' : this.imageBlobData != '' ? encodeURIComponent(this.imageBlobData) : '',
        ...
        /** Authors: Sanjana & Thanvi Date: 31-12-2020 CR No: JFW-28-Dec-03
         * The below code is added to fecth the state list to be displayed for the item type name COUPON
         */
        @wire(getStateList)
        getAllStates;
        ...
        get getStateListToBeDisplayed() {
            ...
            if (this.getStates.data) {
                ...
                // console.log('statelist from backend--->', JSON.stringify(this.getStates.data));
                this.availableStateList = getStateList.allStates;
                // console.log('availableStateList---->',this.availableStateList);
                this.assignedStateList = getStateList.selectedStates;
                //to hold the selected ids if they exist from the assignedStateList
            ...
            // to get the state list for the item type coupoun if the states are not associated
                ...
                // console.log('entered else--->', JSON.stringify(this.availableStateList));
            }

        }
        /** Authors: Sanjana & Thanvi Date: 31-12-2020 CR No: JFW-28-Dec-03
         * The capture the states ids selected by the user.
         */
        handleSelectedStates(event) {
            this.invokeType = 'selectedStateIds';
            this.selectedStateIds = event.target.value;
            this.validatePosItemData();
            // console.log('selectedStates in parent---->', JSON.stringify(this.selectedStateIds));
        }
